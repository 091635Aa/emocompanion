...
本发明提出"超融合"（Ultra Fusion Dynamics, UFD）方法，核心思想是：将静态的角色方向向量v_角色与动态的用户状态向量v_dyn按权重合成为有效方向向量v_eff，并通过稠密乘性重加权（Dense Multiplicative Reranking, DMR）弥补稀疏打分表的OOV短板。
