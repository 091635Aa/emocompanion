# -*- coding: utf-8 -*-
"""P6 评测·裁判阶段：LLM-Judge（本地 7B 手动 bf16 加载，配对盲评 AB 正反各一次，断点续跑）
...
结果路径 = os.path.join(输出目录, "P6_LLMJudge_30.json")
...
def 主():
    print(f"=== P6 LLM-Judge 裁判阶段 {datetime.now().strftime('%H:%M:%S')} ===", flush=True)
    ...
    for 模式 in 模式列表:
        已有 = 结果["配对"].get(模式)
        if 已有 and not 已有.get("未完成"):
            print(f"[{模式}] 已完成（{len(已有.get('明细', []))}条），跳过", flush=True)
            continue
        if 已有 and 已有.get("未完成"):
            print(f"[{模式}] 存在未完成暂存（{len(已有.get('明细', []))}条），删除后重新完整续跑", flush=True)
            del 结果["配对"][模式]
        胜 = 0
        总 = 0
        配对明细 = []
        for i, 项 in enumerate(样本):
            # 让路机制：用户紧急任务标记存在 → 保存状态安全退出（断点续跑恢复）
            if os.path.exists(r"C:\P6临时盘\让路.flag"):
                print("检测到让路标记，保存进度后退出（下次自动续跑）", flush=True)
            ...
            if (i + 1) % 10 == 0:
                ...
                with open(结果路径, "w", encoding="utf-8") as f:
                    json.dump(结果, f, ensure_ascii=False, indent=2)
        win_rate = 胜 / max(总, 1)
        结果["配对"][模式] = {"win_rate": round(win_rate, 4), "胜": 胜, "总": 总, "明细": 配对明细}
        with open(结果路径, "w", encoding="utf-8") as f:
            json.dump(结果, f, ensure_ascii=False, indent=2)
    ...
    for 模式 in 模式列表:
        ...
        print(f"{模式:<10} win_rate={wr:.4f}", flush=True)
