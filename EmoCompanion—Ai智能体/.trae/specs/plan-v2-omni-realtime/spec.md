# EmoCompanion智能体 V2（全模态实时通话）规格

## Why

现有 [tts_studio](file:///f:/EmoCompanion—Ai智能体/tts_studio) 仅提供基础的 Qwen-Audio TTS 语音合成（V1），音色定制停留在 [voice_clone_work](file:///f:/EmoCompanion—Ai智能体/voice_clone_work) 的脚本阶段。用户需要把产品升级为**可实时通话、可打断、可切换音视频、可定制音色、可多模态识别**的完整 Web 应用，并将 V1 冻结归档为 TDS 版本。
...
| V2 | EmoCompanion智能体 | 实时通话 + 定制音色 + 多模态识别 + 交互增强 | 新建独立目录，全中文编码开发 |

V2 架构基线（已与用户确认）：
- **目录形态**：新建独立目录 `f:\EmoCompanion—Ai智能体\EmoCompanion智能体\`，V1 归档至其 `归档/TDS版本/`
...
- 新建 `EmoCompanion智能体/` V2 项目（全中文目录/文件名/变量名/函数名），实现实时通话、定制音色、多模态识别、前端交互增强四大能力
...
- 受影响代码：
  - [app.py](file:///f:/EmoCompanion—Ai智能体/tts_studio/app.py)、[tts.py](file:///f:/EmoCompanion—Ai智能体/tts_studio/tts.py)、[config.py](file:///f:/EmoCompanion—Ai智能体/tts_studio/config.py)、[deepseek.py](file:///f:/EmoCompanion—Ai智能体/tts_studio/deepseek.py)、[static/index.html](file:///f:/EmoCompanion—Ai智能体/tts_studio/static/index.html)
  - [voice_clone_work](file:///f:/EmoCompanion—Ai智能体/voice_clone_work) 全部脚本（select_best_segment/enroll_voice/synthesize/verify_quality/upgrade_voice/synth_emotion）
...
系统 SHALL 按以下目录结构建立 V2 项目（`f:\EmoCompanion—Ai智能体\EmoCompanion智能体\`）：
...
#### Scenario: 目录就绪与中文导入
- **WHEN** 开发人员在 `EmoCompanion智能体/` 下运行 `python 启动入口.py`
...
V2 SHALL 迁移并保留 V1 全部合成能力：模型列表/别名、系统音色/复刻音色、情绪/富语言标签（30 个）、全局风格指令与场景预设、多音字发音纠正、DeepSeek AI 自动标注与指令优化、余额探测。实现基于 [tts.py](file:///f:/EmoCompanion—Ai智能体/tts_studio/tts.py) 与 [app.py](file:///f:/EmoCompanion—Ai智能体/tts_studio/app.py) 中对应逻辑，以中文命名落地到 `核心模块/语音合成.py`、`核心模块/配置持久化.py`、`核心模块/智能助手.py`，合成 API 迁移为 `/api/合成` 等中文路径（同时保留英文路径别名以兼容历史调用，视实现取舍）。
...
**Migration**: 完整拷贝 `tts_studio` 至 `EmoCompanion智能体/归档/TDS版本/`（含源码、requirements、.env 模板、README 说明）；原目录保留不动，仅作历史参考。
