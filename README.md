# 缘圆 AI 情感引擎（yuanyuan-ai-emotion）

> 私有仓库 ｜ 整理日期 2026-08-31 ｜ 本仓库为 `d:\AI情感` 工作区的**源码 + 技术文档**整理快照（不含模型权重、训练数据、商业提案）

缘圆是一套本地化的「AI 角色情感引擎」，覆盖两条主线：
...
| `缘圆_角色挂载与情感注入工程/` | **核心工程**：文本引擎后端、05 三档显存测试实验线、06 Qwen3TTS 外挂（tts_gguf / unified_server / integrated_app / finetuning / calibration）、最终归档与接口文档 |
| `最后版本/` | 一体化全流程应用（启动.py + 核心引擎 + 前端 + 测试） |
| `缘圆—Ai智能体/` | 桌面 AI 智能体（核心模块：后端服务/多模态识别/实时通话/语音合成/音色管理 + TDS 版本归档） |
...
```bash
# ① 文本引擎（三层情感注入 + FastAPI）
cd 缘圆_角色挂载与情感注入工程/04_源码与原型/backend
python app.py            # 自动创建/激活 .venv、下载依赖、启动 8000 端口

# ② 文本+语音一体化服务
cd 缘圆_角色挂载与情感注入工程/06_Qwen3TTS外挂/serve
...
- `缘圆_角色挂载与情感注入工程/07_最终归档/最终归档.md` —— 里程碑归档（M0-M4）
- `缘圆_角色挂载与情感注入工程/07_最终归档/接口文档.md` —— 文本引擎 v1.0 API
...
私有仓库，仅限项目成员访问。专利文档为申报前内部稿件，请勿外传。
