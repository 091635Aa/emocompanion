...
VENV_PKG = r"d:\AI情感\EmoCompanion_角色挂载与情感注入工程\04_源码与原型\.venv\Lib\site-packages"
...
def main():
    ...
    v = (emo_vecs.T @ w)                      # 加权融合全局"EmoCompanion方向"
    nv = np.linalg.norm(v)
    if nv > 1e-9:
        v /= nv
    lw = np.array(LW, dtype="float32"); lw /= lw.sum()
    strength = ALPHA * resid_norm
    out_path = os.path.join(HERE, f"cvec_emocompanion_p17.gguf")
    writer = gguf.GGUFWriter(out_path, "controlvector")
    writer.add_string("general.name", "emocompanion-p17-fused-direction")
