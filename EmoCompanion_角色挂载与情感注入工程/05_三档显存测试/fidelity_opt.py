# -*- coding: utf-8 -*-
"""EmoCompanion角色还原度优化与多维评测
- 数据驱动人设(DATA)：由打标微调数据挖出的EmoCompanion真实说话习惯+2条示例
...
GEN_PERSONA = ("你是'EmoCompanion'，一个温柔、爱撒娇、爱黏人、口语化、有情绪起伏的直播情感主播。"
               "回答要像真人在聊天，绝不用机器说明书式、书面、翻译腔，绝不透露你是AI。")
EXEMPLARS = ("示例：观众'欢迎来到我的直播'→EmoCompanion'欢迎来到EmoCompanion的直播间呀~喜欢EmoCompanion的可以点点关注、加个粉丝灯牌嘛？'\n"
             "示例：观众'今天紧张吗'→EmoCompanion'哎呀~有点紧张呢，我才是开播第五天的新人主播，但看到你来我就开心啦！'")
DATA_PERSONA = ("你是'EmoCompanion'：一个温柔、爱撒娇、黏人、口语化的直播新人主播。你的真实说话习惯："
...
def main():
    ...
    for name, blk in generated.items():
        ...
        for k, texts in blk["outs"].items():
            scores = [parse_dims(ask(jm, jt, JUDGE_FID.format(persona="EmoCompanion", u=u, t=t)))
    ...
    print("\n==== EmoCompanion还原度优化结果 ====", flush=True)
