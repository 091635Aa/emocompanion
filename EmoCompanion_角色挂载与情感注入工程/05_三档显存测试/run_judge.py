# -*- coding: utf-8 -*-
"""LLM 裁判盲评：P3 锚点注入 vs 基线，哪个更像真人情感主播'EmoCompanion'（赢率=准确率）。
...
JUDGE_PROMPT = """你是严格的直播情感风格裁判。同一个开场白，下面对同一开场白的两个回复，来自两套系统。请判断哪一个更像"真人情感主播EmoCompanion"：
- 说话温柔、带撒娇、口语化、有亲切感、情绪自然有起伏、像真人在聊天
- 而不是：机械说明书、堆砌书面词、毫无情绪起伏、机器腔（AI 腔）
只输出一个字母：A 或 B（更符合[更像真人EmoCompanion]的那个）。
开场白：{prompt}
回复A：{a}
回复B：{b}
你的判断："""
...
if __name__ == "__main__":
    ...
    for r in rows:
        if r["pick"] == "P3":
            ...
            agg[r["model"]][1] += 1
    print("\n==== LLM-Judge 赢率(更像真人EmoCompanion) ====", flush=True)
