# -*- coding: utf-8 -*-
"""EmoCompanion 主播化长线对话 CLI 测试
模拟直播间观众 → 主播 多轮互动（同一会话，保持上下文连续），验证：
  - 人设一致（EmoCompanion 温柔/撒娇/口语化/主播话术）
...
sid = s["id"]
print("=" * 70)
print(f"会话: {sid}  |  主播: EmoCompanion（温柔撒娇主播 persona）")
print("=" * 70)

for label, user_msg in SCENES:
    r = post(f"/api/sessions/{sid}/talk", {
        "content": user_msg, "want_tts": False, "role": "EmoCompanion"})
    emo = r.get("emotion", "?")
