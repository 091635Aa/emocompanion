# -*- coding: utf-8 -*-
"""EmoCompanion · 一体化对话台（文本生成 + 本地 TTS + 会话历史 + 挂载切换 + 角色设定）
...
app = FastAPI(title="EmoCompanion · 一体化对话台", version="2.0.0")
app.add_middleware(
...
def _text_engine_env():
    ...
    engine_root = _f.parents[2]               # EmoCompanion_角色挂载与情感注入工程
...
def main():
    ...
    import uvicorn
    _load_roles_file()
    ap = argparse.ArgumentParser(description="EmoCompanion一体化对话台")
    ...
    print(f"[yy-app] EmoCompanion一体化对话台: http://{a.host}:{a.port}  文本引擎={_TEXT_BASE}")
    print(f"[yy-app] 角色: {list(ROLES.keys())} | 挂载: {_mount}")
    print("[yy-app] TTS: 单一外挂方案 llama-tts GGUF(CUDA INT4, RTF≈0.4)", flush=True)
    log("info", "EmoCompanion一体化对话台启动", host=a.host, port=a.port, text_base=_TEXT_BASE)
