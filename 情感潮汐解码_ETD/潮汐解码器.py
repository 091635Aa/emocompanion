...
# 共享解码控制基类所在目录（KV_情感共振解码/核心）
_共享基类目录 = os.path.normpath(os.path.join(
    os.path.dirname(os.path.abspath(__file__)), "..", "KV_情感共振解码", "核心"))
if _共享基类目录 not in sys.path:
    sys.path.insert(0, _共享基类目录)
...
class 潮汐解码器(解码控制基类):
    ...
    def __init__(
        ...
        # AI 腔 token 表：token_id → 抑制权重（v3 双通道，由基类构建）
    ...
    # ──────────────────────────────────────────────
    # ──────────────────────────────────────────────
    # 解码控制（AI腔抑制/身份暴露拦截/口语化引导/长度收尾/句子停止/兜底监测）
    # ──────────────────────────────────────────────
    # 六类解码控制逻辑与词表已收敛至共享基类 解码控制基类（见 base_decoding_controller.py），
    # 本类继承基类实现，仅覆写 _兜底_处置 保留潮汐解码器独有的差异化行为：
    # 触发兜底后 α 衰减；连续 3 次仍退化 → α 归零（纯裸采样）；
    # 未归零时重置 当前目标（强制下一句重新计算）。

    def _兜底_处置(self, 重复率: float, 触发次数: int) -> None:
        """覆写：潮汐解码器版兜底处置 —— 统计计数 + α 衰减；未归零时重置 当前目标。"""
        self.统计["触发兜底"] += 1
        if 触发次数 >= 3:
            self.当前α = 0.0
        else:
