# EmoCompanion文本生成主播化优化 —— 情感向量外挂路由 + 行为层 Spec
...
## What Changes
- **文本引擎外挂路由扩展**：在 [engine.py](file:///d:/AI情感/EmoCompanion_角色挂载与情感注入工程/04_源码与原型/backend/engine.py) 的 `PersonaLayers` 新增"情感向量偏置层"，logits 级注入，主模型权重不变：
...
- **角色包数据扩展**：[build_role_pack.py](file:///d:/AI情感/EmoCompanion_角色挂载与情感注入工程/04_源码与原型/backend/build_role_pack.py) 预计算"8 情感 × V 维"情感向量表 + 角色本身情感向量，运行期零 transformers，热切换角色包即换情感基底。
