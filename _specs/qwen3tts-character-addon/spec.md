...
## Why
现有的角色挂载/情感注入工程聚焦于**文本 LLM 推理期注入**（persona 提示、logits/KV/状态向量注入）。本 spec 面向一个**不同的、互补的能力**：通过**有监督微调（LoRA）**，把某一直播角色（EmoCompanion）的**音色、口语风格、情感韵律**封装为独立、可热插拔、不污染 `Qwen3-TTS-12Hz-1.7B-Base` 原始权重的「角色外挂包」，实现情感标签可控的语音合成。
...
- Affected data: `EmoCompanion_角色挂载与情感注入工程/02_角色参数与数据/微调数据/`（将被清洗、划分、格式化为 Qwen3-TTS 训练集）。
