# -*- coding: utf-8 -*-
"""缘圆 CLI 交互工具 —— 模型/扩展包扫描 + 定向启动 + 多请求格式
...
请求格式支持（chat）:
  1) 空格分隔:    缘圆cli.py chat 晚上好呀 今天开心吗
  2) 引号文本:    缘圆cli.py chat "晚上好呀，今天开心吗"
  3) JSON 格式:   缘圆cli.py chat '{"messages":[{"role":"user","content":"hi"}],"max_new":64}'
  4) 交互模式:    缘圆cli.py chat -i         （多轮持续对话）
  5) 定向启动:    缘圆cli.py chat -m Qwen3-4B-Q4_K_M.gguf -p default 晚上好呀
...
PROJ_ROOT = ENGINE_ROOT.parent                              # 缘圆_角色挂载与情感注入工程
...
def build_engine(model_path, pack_name):
    """按模型+扩展包构造引擎（可复用 engine.YuanyuanEngine 或独立加载）"""
    from engine import YuanyuanEngine
    eng = YuanyuanEngine.get()
...
def main():
    p = argparse.ArgumentParser(description="缘圆 CLI 交互工具")
