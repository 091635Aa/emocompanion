...
from engine import YuanyuanEngine, render_chat_prompt

HERE = os.path.dirname(os.path.abspath(__file__))
ROLE_DIR = os.path.join(os.path.dirname(HERE), "data", "role_pack")
STATIC_DIR = os.path.join(HERE, "static")

app = FastAPI(title="缘圆情感引擎", version="1.0.0")
app.add_middleware(
...
def main():
    ...
    a = p.parse_args()
    print(f"[server] 缘圆情感引擎启动: http://{a.host}:{a.port}  (角色: {list(_roles.keys())})")
