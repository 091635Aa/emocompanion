...
候选(4bit, chat模板真角色扮演, 缘圆人设): Qwen3-4B / DeepSeek-Qwen7B(=Qwen2-7B)
裁判(4bit): Qwen3-8B(与候选错开, 随机A/B去位置偏置)。
指标: LLM-Judge 各β真实赢率 | 缘圆角色保真度(0-100) | 熵/重复/正情绪/吞吐/稳态显存。
...
PERSONA = ("你是'缘圆'，一个温柔、爱撒娇、爱黏人、口语化、有情绪起伏的直播情感主播。"
           "回答要像真人在聊天：会自然流露温柔/泛爱意，会用'呀~''嘛''嘻嘻'这类语气，"
           "有情绪变化，绝不用机器说明书式、书面堆砌、翻译腔，绝不透露你是AI。")
s = Sentiment()
if NOPERSONA:   # 决定性对照：去掉人设提示，检验"情感注入是否独立创作出缘圆口吻"
...
JUDGE_AB = ("你是直播情感风格裁判。针对观众发言'{u}'，有回复A、回复B，来自两套系统。"
            "判断哪一个更像'缘圆'：温柔/爱撒娇/口语化/有情绪起伏/像真人聊天(而非说明书、书面腔、翻译腔、AI腔)。"
            "只输出：A 或 B。\n回复A：{a}\n回复B：{b}\n你的判断：")
JUDGE_FID = ("给下面这条直播回复打'像真人缘圆主播(温柔/撒娇/口语化/有情绪)'的保真分，0-100，只输出数字。"
             "观众发言：{u}\n回复：{t}\n保真分：")
...
def main():
    ...
    for name, blk in cand_out.items():
        ...
        for k, (w, t) in wr.items():
            print(f"      β{k}: LLM-Judge {w}/{t}={w/max(t,1)*100:.0f}%  缘圆保真 基{fid['base']}/{fid[k]}  "
