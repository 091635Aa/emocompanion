...
- **根因**：模型返回的 `response.audio.delta` 事件中音频字段为 `delta`（实测 15 个音频分片全部位于 `delta`），而前端 [通话控制.js](file:///f:/缘圆—Ai智能体/缘圆智能体/前端页面/脚本/通话控制.js#L1128-L1133) 读取 `事件.audio` → 恒为 undefined → **AI 回复只见文字、永远无声**。
- 连接、`session.update`、定制音色（`qwen-omni-vc-yuanyuan-voice-20260802030956459-59e8`，绑定 `qwen3.5-omni-plus-realtime`）、文字往返均已在协议层验证通过。

## What Changes

- 修复 [通话控制.js](file:///f:/缘圆—Ai智能体/缘圆智能体/前端页面/脚本/通话控制.js) 的 `response.audio.delta` 处理：音频取 `事件.delta`（保留 `事件.audio` 作为兼容回退）。
...
- 受影响代码：
  - [通话控制.js](file:///f:/缘圆—Ai智能体/缘圆智能体/前端页面/脚本/通话控制.js)（前端音频播放字段）
  - [语音合成.py](file:///f:/缘圆—Ai智能体/缘圆智能体/核心模块/语音合成.py)（在线音色查询输出 target_model）
  - [音色管理.py](file:///f:/缘圆—Ai智能体/缘圆智能体/核心模块/音色管理.py)（通话可用按 target_model 复核）
...
#### Scenario: 通话自动使用定制音色
- **WHEN** 用户进入通话且存在绑定实时模型的定制音色（当前为 `qwen-omni-vc-yuanyuan-voice-20260802030956459-59e8`）
