...
class 目标决策器:
    ...
    def 计算目标(self, 用户当前=None, 思考链文本: str = "", 指令: str = "",
                角色=None, 轮次: int = 0, 角色权重: float = 0.7) -> 锚点目标:
        ...
        角色（SubTask 6b 新增·角色感知）：角色名 str（查 角色锚点基调表）或
            ...
            v_target = 角色权重·v_角色 + (1-角色权重)·v_用户（角色基调主导方向、
            用户当前情感辅助微调），角色基调跨轮固定、用户部分每轮更新；
            未命中映射表时自动跳过 → 行为与不传角色完全一致（向后兼容）。
        角色权重：角色基调占比（默认 0.7；角色扮演长会话可调高至 0.85 增强跨轮一致）。
