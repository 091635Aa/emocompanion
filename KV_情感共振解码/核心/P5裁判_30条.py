# -*- coding: utf-8 -*-
"""P5 情感共振解码器 LLM-Judge —— 裁判阶段（本地 7B 手动 bf16 加载，断点续跑）

协议同 P4 三正交叠加报告：配对盲评 AB 正反各一次。裁判提示词/加载路径与 ETD 一致。
每调用异常重试 2 次；每 5 样本 gc + 清显存；增量保存，支持断点续跑。
"""
import os
import sys
import json
...
生成路径 = os.path.join(输出目录, "P5_生成_30.json")
结果路径 = os.path.join(输出目录, "P5_LLMJudge_30.json")
...
def main():
    print(f"=== P5 LLM-Judge 裁判阶段 {datetime.now().strftime('%H:%M:%S')} ===", flush=True)
    with open(生成路径, "r", encoding="utf-8") as f:
        数据 = json.load(f)
    样本 = 数据["回复"]
    模式列表 = 数据["模式"]
    print(f"样本数={len(样本)} 模式={模式列表} 模式种子={数据.get('模式种子', {})}", flush=True)
