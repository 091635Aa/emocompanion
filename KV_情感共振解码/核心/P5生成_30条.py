# -*- coding: utf-8 -*-
"""P5 情感共振解码器 30 条样本生成 —— 裸 / 锚点P4 / 超融合DMR / P5-KER / P4+P5

每模式独立种子（用户要求），协议同 P4 测试（纯净模式、temperature=1.0、
top_p=0.9, top_k=50, repetition_penalty=1.05, max_new_tokens=256）。
"""
import os
import sys
import json
...
结果路径 = os.path.join(输出目录, "P5_生成_30.json")
...
def main():
    print(f"=== P5 生成阶段 {datetime.now().strftime('%H:%M:%S')} ===", flush=True)
    ...
    结果 = {"模型": "Qwen2.5-1.5B-Instruct", "模式种子": 模式种子, "模式": 模式列表,
    ...
    for i, 项 in enumerate(样本):
        ...
        for 名称 in 模式列表:
            ...
            if 名称 == "裸":
                文本 = 裸生成(模型, 分词器, 提示, 种子)
                条目["回复"][名称] = {"文本": 文本, "统计": {"种子": 种子}}
            else:
                文本, 统计 = 方案生成(模型, 分词器, 解码器[名称], 提示, 种子)
                条目["回复"][名称] = {"文本": 文本, "统计": 统计}
        结果["回复"].append(条目)
        if (i + 1) % 10 == 0:
